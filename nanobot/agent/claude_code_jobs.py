"""Claude Code (CC) job orchestration for long-running code tasks."""
...
@dataclass
class ClaudeCodeJob:
    """Persisted state for one Claude Code execution."""
...
class ClaudeCodeJobManager:
    """Manage long-running Claude Code jobs outside the main agent loop."""
    ...
    def __init__(self, workspace: Path, bus: MessageBus):
        ...
        self.bus = bus
        self.jobs_dir = ensure_dir(workspace / ".nanobot" / "cc-jobs")
    ...
    async def delegate(
        ...
        *,
        ...
        """Start a new Claude Code job or reuse/resume an existing logical task."""
        ...
        async with self._lock:
            ...
            if existing is not None:
                if existing.is_active and self._job_pid_running(existing):
                    ...
                    return (
                        f"Claude Code job `{existing.job_id}` is already running"
                        + (f" for `{resume_key}`." if resume_key else ".")
                    )
                if existing.status == "completed":
                    return (
                        f"Claude Code job `{existing.job_id}` already completed."
                        " Use `cc_status` to inspect the result or choose a new resume_key."
                    )
                if existing.session_id:
                    await self._launch_job(existing, self._build_resume_prompt(existing, None), resume=True)
                    return (
                        f"Resumed Claude Code job `{existing.job_id}`"
                        + (f" for `{resume_key}`." if resume_key else ".")
                    )
                ...
                return (
                    f"Restarted Claude Code job `{existing.job_id}`"
                    + (f" for `{resume_key}`." if resume_key else ".")
                )
            ...
            return (
                f"Started Claude Code job `{job.job_id}`"
                + (f" for `{resume_key}`." if resume_key else ".")
                + " Use `cc_status` to check progress."
            )
    ...
    async def resume(
        ...
        *,
        ...
        """Resume an interrupted or failed Claude Code job."""
        await self.restore_pending_jobs()
        async with self._lock:
            ...
            if job.is_active and self._job_pid_running(job):
                ...
                return f"Claude Code job `{job.job_id}` is already running."
            ...
            return f"{action} Claude Code job `{job.job_id}`."
    ...
    async def close(self) -> None:
        """Stop in-process monitors without terminating Claude Code child processes."""
    ...
    def _save_job(self, job: ClaudeCodeJob) -> None:
        self._state_path(job.job_id).write_text(
            json.dumps(asdict(job), ensure_ascii=False, indent=2),
            encoding="utf-8",
        )

    def _load_job(self, job_id: str) -> ClaudeCodeJob | None:
        path = self._state_path(job_id)
        if not path.exists():
            return None
        try:
            return ClaudeCodeJob(**json.loads(path.read_text(encoding="utf-8")))
        except Exception:
            logger.exception("Failed to load Claude Code job state {}", job_id)
            return None

    def _load_all_jobs(self) -> list[ClaudeCodeJob]:
        jobs: list[ClaudeCodeJob] = []
        for state_path in self.jobs_dir.glob("*/state.json"):
            try:
                jobs.append(ClaudeCodeJob(**json.loads(state_path.read_text(encoding="utf-8"))))
            except Exception:
                logger.exception("Failed to read Claude Code job state from {}", state_path)
        jobs.sort(key=lambda item: item.updated_at, reverse=True)
        return jobs

    def _resolve_job(self, job_id: str | None, resume_key: str | None) -> ClaudeCodeJob | None:
        if job_id:
    ...
    async def _launch_job(self, job: ClaudeCodeJob, prompt: str, *, resume: bool) -> None:
    ...
    async def _create_process(
        ...
        *,
        ...
        logger.info("Launching Claude Code job in {}: {}", cwd, command)
    ...
    def _build_command(self, job: ClaudeCodeJob, prompt: str, *, resume: bool) -> list[str]:
        last_path = str(self._last_message_path(job.job_id))
        base = self._cc_prefix()
    ...
    @staticmethod
    def _cc_prefix() -> list[str]:
        if os.name == "nt":
            return ["cmd", "/d", "/c", "claude"]
        return ["claude"]
    ...
    async def _monitor_job(self, job_id: str, *, resumed: bool = False) -> None:
        ...
        try:
            ...
            logger.exception("Claude Code monitor crashed for {}", job_id)
            async with self._lock:
                job = self._load_job(job_id)
                if job is not None:
                    job.status = "failed"
                    job.last_error = "Claude Code monitor crashed"
                    job.updated_at = timestamp()
                    self._save_job(job)
        finally:
    ...
    async def _finalize_job(self, job_id: str, *, exit_code: int | None) -> None:
        async with self._lock:
            ...
            if exit_code == 0 or (exit_code is None and final_message):
                ...
                job.status = "failed"
                job.last_error = f"Claude Code exited with code {exit_code}" if exit_code is not None else "Claude Code ended unexpectedly"
            else:
                job.status = "interrupted" if exit_code is None else "failed"
                if exit_code is not None:
                    job.last_error = f"Claude Code exited with code {exit_code}"
                elif not job.last_error:
                    job.last_error = "Claude Code stopped before producing a final message"
            job.updated_at = timestamp()
            self._save_job(job)
    ...
    async def _announce_completion(self, job_id: str) -> None:
        ...
        await self.bus.publish_inbound(
            InboundMessage(
                ...
                content=content,
                metadata={"cc_job_id": job.job_id},
                session_key_override=job.session_key,
            )
        )

    def _write_spec(self, job: ClaudeCodeJob) -> None:
        lines = [
            "# Claude Code Job",
            "",
            f"- Job ID: {job.job_id}",
    ...
    @staticmethod
    def _build_resume_prompt(job: ClaudeCodeJob, extra_prompt: str | None) -> str:
        lines = [
    ...
    @classmethod
    def _display_status(cls, job: ClaudeCodeJob) -> str:
    ...
    def _read_last_message(self, job: ClaudeCodeJob) -> str:
        ...
        try:
            ...
            logger.exception("Failed to read final Claude Code output for {}", job.job_id)
            return ""
        if len(content) <= 400:
            return content
        return content[:400] + "..."
    ...
    def _extract_session_id(line: str) -> str | None:
        """Extract session_id from CC JSON output."""
        try:
            data = json.loads(line)
        except Exception:
            return None
        # CC JSON output has session_id at top level
        session_id = data.get("session_id")
        if session_id and isinstance(session_id, str):
            return session_id
        return None

    def _job_pid_running(self, job: ClaudeCodeJob) -> bool:
    ...
    def _not_found(job_id: str | None, resume_key: str | None) -> str:
        if job_id:
            return f"Error: Claude Code job `{job_id}` not found."
        if resume_key:
            return f"Error: No Claude Code job found for resume_key `{resume_key}`."
