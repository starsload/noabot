...
def test_desktop_voice_chat_runs_until_max_turns(monkeypatch, tmp_path: Path) -> None:
    config = Config()
    ...
    result = runner.invoke(app, ["desktop-voice-chat", "--max-turns", "2"])

    assert result.exit_code == 0
    assert "Chat turn 1" in result.stdout
    assert "Chat turn 2" in result.stdout
    assert "transcript-1" in result.stdout
    assert "transcript-2" in result.stdout
    assert seen["turns"] == 2


def test_desktop_voice_chat_stop_on_empty_transcript(monkeypatch, tmp_path: Path) -> None:
    config = Config()
    config.agents.defaults.workspace = str(tmp_path / "workspace")
    seen: dict[str, int] = {"turns": 0}
    ...
    class _FakeRunner:
        ...
        async def run_turn_from_capture(self, capture, *, duration_s=None, language=None, voice=None):  # noqa: ANN001
            ...
            seen["turns"] += 1
            idx = seen["turns"]
            transcript = "" if idx == 1 else f"transcript-{idx}"
            return type(
                "VoiceTurnResult",
                (),
                {
    ...
    result = runner.invoke(app, ["desktop-voice-chat", "--stop-on-empty"])
