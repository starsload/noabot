...
def _mk_loop() -> AgentLoop:
    ...
    # Custom 59b3cb1d: _save_turn tags source_channel on owner shared sessions.
    loop.sessions = SimpleNamespace(is_owner_session=lambda key: False)
    return loop
...
# ---------------------------------------------------------------------------
# noabot custom persistence behavior (re-applied from dev-clean-era commits
# 53a4dbf1 / 59b3cb1d / 7681072a onto the upstream runtime-context design).
# ---------------------------------------------------------------------------
...
def test_save_turn_replaces_tool_inline_images_with_placeholders() -> None:
    ...
    assert session.messages[1]["metadata"]["had_inline_images"] is True
...
def test_save_turn_keeps_image_placeholder_after_runtime_strip() -> None:
    loop = _mk_loop()
    session = Session(key="test:image-runtime-lead")
    runtime = ContextBuilder._RUNTIME_CONTEXT_TAG + "\nCurrent Time: now (UTC)"
