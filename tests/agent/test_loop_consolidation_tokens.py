...
def _make_session(loop: AgentLoop, entries: list[tuple[str, str]]) -> None:
    session = loop.sessions.get_or_create("cli:test")
    session.messages = [
        {"role": role, "content": content, "timestamp": f"2026-01-01T00:00:{idx:02d}"}
        for idx, (role, content) in enumerate(entries)
    ]
...
def test_memory_consolidator_uses_provider_generation_max_tokens(tmp_path) -> None:
    from nanobot.providers.base import GenerationSettings
    provider = MagicMock()
    provider.get_default_model.return_value = "test-model"
    ...
    assert loop.memory_consolidator.max_completion_tokens == 8192


def test_fit_history_within_budget_trims_to_next_user_turn(tmp_path, monkeypatch) -> None:
    loop = _make_loop(tmp_path, estimated_tokens=0, context_window_tokens=100)
    loop.memory_consolidator.max_completion_tokens = 20
    loop.memory_consolidator._SAFETY_BUFFER = 0
    ...
    def fake_estimate(_provider, _model, messages, _tools):
    ...
    monkeypatch.setattr(memory_module, "estimate_prompt_tokens_chain", fake_estimate)

    fitted, estimated, source = loop.memory_consolidator.fit_history_within_budget(
        history,
    ...
    assert [message["content"] for message in fitted] == ["u2", "a2"]
    assert estimated == 80
    assert source == "test"


@pytest.mark.asyncio
async def test_process_direct_uses_trimmed_history_when_budget_still_exceeded(tmp_path, monkeypatch) -> None:
    loop = _make_loop(tmp_path, estimated_tokens=0, context_window_tokens=100)
    loop.memory_consolidator.max_completion_tokens = 20
    loop.memory_consolidator._SAFETY_BUFFER = 0
    ...
    async def track_llm(*, messages, **kwargs):
